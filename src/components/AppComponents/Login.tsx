...
const Login = ({setShowLogin}: {setShowLogin: React.Dispatch<React.SetStateAction<boolean>>}) => {
	...
	return (
		<div className="login-popup fixed z-50 w-screen h-screen top-0 left-0 bg-black/90 grid">
			<form action="" className="login-popup-container place-self-center bg-white text-[#808080] flex flex-col gap-6.5 py-6.5 px-7.5 rounded-lg text-sm animate-[fadeIn_0.5s] h-fit" style={{width: "max(23vw, 330px)"}}>
        ...
        >
				...
				{currentState === "Login" ? (
					<p className="text-center">
						...
						<span className="text-tomato cursor-pointer" onClick={() => setCurrentState("Sign Up")}>
							Create An Account
						</span>
					</p>
				) : (
					<p className="text-center">
						...
						<span className="text-tomato cursor-pointer" onClick={() => setCurrentState("Login")}>Login</span>
					</p>
				)}
			</form>
		</div>
